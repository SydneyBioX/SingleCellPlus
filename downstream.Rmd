...
The `monocle` method is a well-established psuedo-time trajectory reconstruction method from citation. You can learn more about the theoretical construction of `monocle` here. In terms of computations, `monocle` requires some psuedo-time modelling on each gene and then markers are selected to perform psuedo-time ordering. 
...
monocle_CellDataSet = estimateDispersions(monocle_CellDataSet)
...
# plot_genes_in_pseudotime(monocle_CellDataSet["Epcam",], color_by="Time")
```
