...
plot_sce_cellTypes_batch = function(sce, exprs_values){
  ...
  tsne_mat = sce_tsne@reducedDims@listData$TSNE
  colnames(tsne_mat) = c("tsne1", "tsne2")
  tsne_df = bind_cols(data.frame(tsne_mat), as.data.frame(colData(sce_tsne))) %>% 
    tibble::rowid_to_column("cellNumber")
  
  ########### Using plotly ################
  # 
  # p1 = plotly::plot_ly(data = tsne_df, x = ~tsne1, y = ~tsne2,
  #                      type = "scatter",
  #                      mode = "markers",
  #                      color = ~cellTypes,
  #                      colors = "Set1")
  # 
  # p2 = plotly::plot_ly(data = tsne_df, x = ~tsne1, y = ~tsne2,
  #                      type = "scatter",
  #                      mode = "markers",
  #                      color = ~batch,
  #                      colors = "Dark2") 
  # 
  # plotly::subplot(p1, p2, titleX = T, titleY = T)
  
  ########### Using scater::plotTSNE ################
  ...
  #   plotly::layout(title = "")
  # 
  # plotly::subplot(sce_tsne_cellTypes2, 
  #                 sce_tsne_batch2)
  
  ########### Using ggiraph::geom_point_interactive ################
  
  sce_tsne_cellTypes = tsne_df %>% 
    ...
    labs(title = paste0(exprs_values, ", coloured by cell types"))
  
  sce_tsne_batch = tsne_df %>% 
    ggplot(aes(x = tsne1, y = tsne2, 
               colour = batch,
               tooltip = batch,
               data_id = cellNumber)) +
    ...
    ggiraph::geom_point_interactive() +
    labs(title = paste0(exprs_values, ", coloured by batch"))

  
  ggiraph::girafe(code = print(sce_tsne_cellTypes + sce_tsne_batch),
...
scater::plotPCA(sce_combine, 
                colour_by = "cellTypes",
                run_args = list(exprs_values = "logcounts"))

plot_sce_cellTypes_batch(sce_combine, exprs_values = "logcounts")
```
...
```{r, eval = FALSE}
saveRDS(sce_scMerge, file = "data/liver_scMerge_2019_May_29.RDS")
```
...
```{r}
plot_sce_cellTypes_batch(sce = sce_scMerge2, 
...
```{r}
plot_sce_cellTypes_batch(sce = sce_scMerge3, 
