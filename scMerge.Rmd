...
You can learn more about a `SingleCellExperiment` object here. In brief terms, in a `SingleCellExperiment` object, it stores three things: 
...
grid.arrange(tsne_logcounts_cellTypes, tsne_logcounts_batch, nrow = 1)
```
...
grid.arrange(tsne_scMerge_supervised_cellTypes, tsne_scMerge_supervised_batch, nrow = 1)
```
...
The answer is yes! In the absence of true cell type information, `scMerge` will perform clustering in the background to assigne the cells with cluster labels. These cluster labels closely mimicks the true cell type labels and a similar performance can be attained. 
...
grid.arrange(tsne_scMerge_unsupervised_cellTypes, tsne_scMerge_unsupervised_batch, nrow = 1)
```
...
grid.arrange(tsne_scMerge_semisupervised_cellTypes, tsne_scMerge_semisupervised_batch, nrow = 1)
```
...
grid.arrange(tsne_scMerge_unsupervised_hept, tsne_scMerge_semisupervised_hept, nrow = 1)
```
...
grid.arrange(tsne_scMerge_fast_cellTypes, tsne_scMerge_fast_batch, nrow = 1)
```
