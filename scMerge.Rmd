...
supervised_cluster = metadata(sce_scMerge)[["scRep_res"]] %>% 
  apply(1, which.max)
```
...
unsupervised_cluster = metadata(sce_scMerge)[["scRep_res"]] %>% 
  apply(1, which.max)

sce_pca = scater::runPCA(sce_scMerge, exprs_values = "scMerge_unsupervised")
pca_mat = sce_pca@reducedDims@listData$PCA
colnames(pca_mat) = c("pca1", "pca2")
pca_df = bind_cols(data.frame(pca_mat), as.data.frame(colData(sce_pca))) %>% 
  mutate(
    supervised_cluster = as.factor(supervised_cluster), 
    unsupervised_cluster = as.factor(unsupervised_cluster))

pca_df_summ = pca_df %>% 
  group_by(supervised_cluster, cellTypes, unsupervised_cluster) %>% 
  dplyr::tally()

pca_df_summ

pca_df_summ %>% 
  ggplot(aes(x = unsupervised_cluster, 
             y = cellTypes,
             fill = n, 
             label = n)) +
  geom_tile() +
...
<!-- ```{r} -->
<!-- scater::plotExpression(sce_scMerge[,sce_scMerge$cellTypes == "Hepatoblast"], -->
