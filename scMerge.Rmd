...
plot_sce_cellTypes_batch = function(sce, exprs_values){
  ...
  #                      colors = "Dark2")
  # 
  # plotly::subplot(p1, p2, shareY = TRUE, shareX = TRUE)
...
```{r}
table(sce_combine$cellTypes, sce_combine$batch, sce_combine$stage)

library(janitor)

janitor::tabyl(
  colData(sce_combine) %>% as.data.frame(),
  cellTypes, batch, stage)
...
```{r, eval = FALSE}
saveRDS(sce_scMerge, file = "data/liver_scMerge.RDS")
```
